...
// Packed-SWAR INVERSE Dilithium NTT (n=256, q=8380417, negacyclic,
// Gentleman-Sande), companion to the forward nttFwV3 in ZZZ_NttVariants.sol.
// Oracle: nttInv in test/vendor/ZKNOX_NTT_dilithium.sol (including the final
// n^{-1} = N_MINUS_1_MOD_Q scaling). Layout: 64 words, 4 coefficients per
// word in 64-bit lanes (lane l of word i = coeff 4i+l). Input lanes may be
// raw lazy-accumulator values (<= ACC_ENTRY = 4(q-1)(17q-1) + q*2^28 < 2^53);
// the fused layer-1/2 block folds the reduction in via scalar mulmod/addmod
// (S14), and the output is canonical (< q).
//
// GS butterfly in lazy lanes:  a[j] <- U+V ;  a[j+t] <- (U + Kq - V)*S.
// The multiplied (difference) lane is reduced to < 2q per
// layer; the sum lane is NOT multiplied, so it is left unreduced and its
// bound DOUBLES per layer. Exact lane bounds (verified by an exhaustive
// bound-tracking model and by the tests):
//
//   entering layer L (t = 2^{L-1}):  every lane < 2^{L-1} * q  (K_L = 2^{L-1})
//   worst multiply input: (U + Kq - V) < 2^L * q, so the per-lane product
//   is < 2^L * q * (q-1); the maximum over all layers is at L=7 (L8 products
//   are reduced with a direct per-lane mod, see below):
//       128*q*(q-1) = 8_989_616_731_324_416 < 2^53.
//
// TWO LANE-LOCAL BARRETT STEPS cover everything — NO extra reduction passes on
// the sum lanes are ever needed, and no lane is ever spread:
//   step 1  x1 := x  - q*floor(x *MU33/2^33)   MU33 = floor(2^33/q) = 1025,
//                                              d33 = 2^33 - MU33*q = 7_167
//   step 2  r  := x1 - q*floor(x1     /2^23)   floor(2^23/q) == 1, multiply
//                                              elided
//   x1 <= q-1 + x*d33/2^33 (<= 7_508_854_654 over this domain, so step 2's
//   quotient is <= 895), and r ≡ x (mod q) with r < 2q for every
//       x < 10_285_325_456_994_078  (~1.03e16, where r == 2q exactly),
//   with qhat = floor(x*MU33/2^33) <= 1_072_692_352 < 2^31 (QHATM31-safe) and
//   x*MU33 <= 2^63.0 < 2^64 (lane-local).
//   128q(q-1) < 1.03e16 with 14% margin. Fuzzed in testInvBarrettDomain.
//
// Layer map (inverse of the forward's): L1 t=1 and L2 t=2 are in-word and
// FUSED (one load/store per data word); L3..L6 (t=4..32) are word-aligned
// packed butterflies; L7 t=64 and L8 t=128 are FUSED with the n^{-1} scaling
// folded in: the single L8 twiddle is premultiplied by n^{-1}
// (S8P = psirev_inv[1]*NINV mod q = 0xb662) for the difference lanes, the sum
// lanes get one shared NINV multiply, and both are canonicalized with a
// per-lane mod (inputs < 256q * q < 2^55), so the output is canonical [0,q).
pragma solidity ^0.8.25;

uint256 constant Q = 8380417; // Dilithium prime, 23 bits
// ⚠ SAFETY MARGIN (symbolic-execution finding; guarded by formal/z3/verify_all.py C11a-c,
// S13). The two-step reduction below is correct for every input up to
// x = 10,285,325,456,994,078 and WRONG at that value (r = 2q exactly there; minimality
// machine-proved). This inverse transform's worst product is 128*q*(q-1) =
// 8,989,616,731,324,416 — only **1.144x** below the cliff. ONE additional unreduced layer
// doubles lane growth and would therefore BREAK the field arithmetic silently.
// Do not raise the lane bound without re-deriving MU/shift and re-running the guard.
