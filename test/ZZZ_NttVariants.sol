...
// Benchmark variants of the Dilithium forward NTT (n=256, q=8380417) exploring
// the realistic gas floor for a 256-point negacyclic NTT in pure EVM.
//
//  V2: same one-coefficient-per-word layout as test/vendor/ZKNOX_NTT_dilithium.sol:nttFw,
//      but hand-tightened Yul: hoisted twiddle-word loads (one mload per 8 groups),
//      inner butterfly loop unrolled x4, last layers specialized/unrolled,
//      no recomputed base offsets. Bit-exact same outputs as nttFw.
//
//  V3: packed SWAR layout, 4 coefficients per 256-bit word in 64-bit lanes.
//      Shared-scalar twiddle multiply: one MUL handles 4 lanes (lane < 2^41 and
//      S < 2^23 keeps per-lane products < 2^64). Products are lazily reduced to
//      < 2q per lane by TWO LANE-LOCAL BARRETT STEPS -- coarse
//      MU33 = floor(2^33/q) = 1025, then the unit step floor(2^23/q) == 1 --
//      whose every intermediate product stays under 2^64, so all four lanes
//      reduce in place with no spreading and no repacking; sums/differences are
//      accumulated UNREDUCED across all 8 layers:
//        lane bound after layer L is (2L+1)*q  =>  <= 17q < 2^28 at the end,
//        so every twiddle product is < 15q*(q-1) < 2^50, 9.76x below the first
//        input at which the two steps fail to land under 2q (verified vs x % q).
//      Layers t=128..4 are word-aligned packed butterflies, run as two
//      RADIX-8 FUSED passes (L1+L2+L3, L4+L5+L6: one octet of eight words
//      loaded once, three layers on the stack, one store); t=2 and t=1 operate
//      within a word in a third fused pass whose L8 scalar products use the EVM's
//      native mulmod. There is NO final canonicalization pass: the output is
//      deliberately LAZY (< 17q), which is exactly the domain the lazy matvec
//      accumulator admits (O7/O8). Verified congruent to nttFw output.
//
//  Pointwise MAC kernels (c[i] += a[i]*b[i] mod q, 256 coeffs) in both layouts,
//  exact and lazy-accumulation (relevant to the A*z matvec: 4 accumulations per
//  output coefficient, products < 2^46 so 4 fit a 64-bit lane / a full word).
...
// prof: >= 5 words; snapshot before pass A, then one after each of the FOUR
// radix-4 fused passes (L1+L2, L3+L4, L5+L6, L7+L8). These markers are the
// CODE anchors Z3 obligation C16 slices its layer blocks on.
// ---------------------------------------------------------------------------
